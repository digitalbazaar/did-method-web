...
export class DidWebDriver {
  ...
  async fromKeyPair({url, verificationKeyPair, keyAgreementKeyPair} = {}) {
    ...
    for(const [vr, value] of Object.entries(didDocument)) {
      if(typeof value === 'string') {
        if(value.startsWith('did:key:')) {
          const fragmentIdx = value.indexOf('#');
          didDocument[vr] = did + value.slice(fragmentIdx);
        }
      } else if(typeof value?.id === 'string' &&
        value.id.startsWith('did:key:')) {
        const fragmentIdx = value.id.indexOf('#');
        value.id = did + value.id.slice(fragmentIdx);
  ...
  async get({did, url, fetchOptions = {}} = {}) {
    ...
    // in the did doc
    if(fragment) {
      return getKey({didDocument: data, keyIdFragment: fragment});
    }
