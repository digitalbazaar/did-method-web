...
export function didToUrl(did) {
  ...
  return {baseUrl, fullUrl, searchParams, search, fragment: hash};
}
...
function _encodePaths(paths) {
  ...
  // remove the fileSuffix, then possibly did.json, then split on "/",
  // & remove empty strings
  const _paths = paths.replace(fileSuffix, '').replace(didFile, '').
