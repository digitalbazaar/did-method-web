...
export function didToUrl(did) {
  ...
  return {baseUrl, searchParams, search, fragment: hash};
}
 ...
 * on "/", uri encodes them, and then rejoins them with ":"
 *
 * @param {string} paths - The paths to encode.
 *
 * @return {string} The paths in did format.
 */
function _encodePaths(paths) {
  if(paths === '/') {
      return '';
  }
  return paths.split(/\//).map(encodeURIComponent).join(':');
}
